	...
	const handleSubmit = useCallback(
		async (event: SubmitEvent) => {
			...
			try {
				...
				downloadFile(filename, JSON.stringify(virtualFs), "application/json");
				setStatus(undefined);
			} catch (error) {
	...
	return (
		<>
			<p>
				...
				writes the stories for you — greatest-players lists, franchise histories,
				rivalries, season retrospectives, game recaps, and more. Unlike the
				on-device recap/season-story features, this puts a frontier model behind
				the writing.
			</p>
			<p>
				...
				and a <code>pull_games.py</code> helper — all embedded. Open the workflow
				doc first; write the foundational pieces (greatest players/teams/busts/
				rivalries) before the dependent ones so the whole corpus stays
				consistent.
			</p>
