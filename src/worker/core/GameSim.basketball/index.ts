...
class GameSim extends GameSimBase {
	...
	getShotInfo({
		...
		if (tipInFromOutOfBounds && passer !== undefined) {
			...
			// Scheme fit: does this team's shooting personnel back up its three-point rate?
			probMake *= this.team[this.o]!.schemeFit?.threePointer ?? 1;
		} else {
			...
			if (r1 > r2 && r1 > r3) {
				...
				// Scheme fit: does this team's finishing ability back up its rim-attack rate?
				probMake *= this.team[this.o]!.schemeFit?.atRim ?? 1;
			} else {
				// Post up
