...
describe("projectPlayer", () => {
	test("trims to narrative fields, sums regular-season career totals, and surfaces cross-refs", () => {
		const p = rawPlayer({
			...
			stats: [
				{ season: 2011, tid: 7, playoffs: false, gp: 70, min: 2000, pts: 1000, trb: 300, ast: 200, ws: 6, vorp: 2, per: 18, ewa: 6 },
				{ season: 2011, tid: 7, playoffs: true, gp: 10, min: 350, pts: 180, trb: 50, ast: 30, ws: 1, vorp: 0.5, per: 19, ewa: 1 },
				{ season: 2012, tid: 3, playoffs: false, gp: 0, min: 0, pts: 0, trb: 0, ast: 0, ws: 99, vorp: 0, per: 0, ewa: 0 },
			],
		});
	...
	test("hof and retiredYear normalize to clean types", () => {
		const active = projectPlayer(rawPlayer({ hof: undefined, retiredYear: null }));
		assert.isFalse(active.hof);
		assert.isUndefined(active.retiredYear);
...
describe("projectTeams + attachTitles", () => {
	const dynasty = rawTeam({
		...
		did: 0,
		seasons: [
			{ tid: 0, season: 2000, won: 60, lost: 22, tied: 0, otl: 0, playoffRoundsWon: 4 },
			{ tid: 0, season: 2001, won: 58, lost: 24, tied: 0, otl: 0, playoffRoundsWon: 4 },
			{ tid: 0, season: 2002, won: 40, lost: 42, tied: 0, otl: 0, playoffRoundsWon: 1 },
		],
	});
	const rival = rawTeam({
		tid: 1,
		did: 0,
		seasons: [
			{ tid: 1, season: 2000, won: 55, lost: 27, tied: 0, otl: 0, playoffRoundsWon: 3 },
			{ tid: 1, season: 2001, won: 54, lost: 28, tied: 0, otl: 0, playoffRoundsWon: 3 },
			{ tid: 1, season: 2002, won: 50, lost: 32, tied: 0, otl: 0, playoffRoundsWon: 4 },
		],
	});
	const otherDiv = rawTeam({ tid: 2, did: 1, seasons: [{ tid: 2, season: 2002, won: 41, lost: 41, tied: 0, otl: 0, playoffRoundsWon: 0 }] });
